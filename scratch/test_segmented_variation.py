...
# Simulate segmented drawing like HTML5 canvas / SwiftUI
# 1. MARKER
pts_marker = []
for x in range(80, 820, 4):
    y = 130 + math.sin((x - 80) * 0.018) * 14
    pts_marker.append((float(x), float(y)))

cum_dist = 0
for i in range(len(pts_marker) - 1):
    x0, y0 = pts_marker[i]
    x1, y1 = pts_marker[i+1]
    seg_dist = math.hypot(x1 - x0, y1 - y0)
    cum_dist += seg_dist

    # Ink width modulation:
    ink_mod = math.sin(cum_dist * 0.038) * 3.8 + math.sin(cum_dist * 0.085) * 1.8
    base_w = max(6.5, 14.5 - min(cum_dist * 0.012, 4.5))
    marker_w = max(4.0, min(18.5, base_w + ink_mod))
    draw_line(x0, y0, x1, y1, marker_w, blue, 245)

# 2. PAINT TUBE
pts_paint = []
for x in range(80, 820, 4):
    y = 310 + math.sin((x - 80) * 0.015) * 12
...
for i in range(len(pts_paint) - 1):
    ...
    phase = math.sin(cum_dist * 0.032) * 0.75 + math.sin(cum_dist * 0.012) * 0.25

    if phase > 0.08:
        # Thick solid impasto phase! (Furrow closed)
        solid_w = 14.0 + (phase - 0.08) * 9.0 # up to ~22px
        draw_line(x0, y0, x1, y1, solid_w, red, 245)
    else:
        # Furrow opens up!
        furrow_factor = min(1.0, (0.08 - phase) / 0.8) # 0 to 1
        
        # Upper ribbon
        upper_w = 11.0 - furrow_factor * 2.5 # ~8.5 to 11
        upper_off = 2.0 + furrow_factor * 3.2
        draw_line(x0 + nx * upper_off, y0 + ny * upper_off,
                  x1 + nx * upper_off, y1 + ny * upper_off,
                  upper_w, red, 242)

        # Lower ribbon
        lower_w = 5.5 - furrow_factor * 1.5 # ~4 to 5.5
        lower_off = -3.5 - furrow_factor * 4.2
        draw_line(x0 + nx * lower_off, y0 + ny * lower_off,
                  x1 + nx * lower_off, y1 + ny * lower_off,
                  lower_w, red, 235)

        # Bristle fiber in the furrow
        if furrow_factor > 0.35:
            fiber_off = -1.2
        ...
        if furrow_factor > 0.5:
            wisp_off = -10.5
            wisp_a = int(furrow_factor * 120)
            draw_line(x0 + nx * wisp_off, y0 + ny * wisp_off,
                      x1 + nx * wisp_off, y1 + ny * wisp_off,
                      1.8, red, wisp_a)

write_png('/Users/rileytechstudio/Documents/Gemini/App/scratch/test_segmented_variation.png', w, h, bytes(canvas))
print("Rendered test_segmented_variation.png successfully!")
